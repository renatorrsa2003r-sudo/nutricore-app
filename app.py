...
def executar_chamada_ia(prompt: str, chave_api: Optional[str] = None) -> dict:
    ...
    if not key:
        raise HTTPException(
            status_code=400,
            detail="Chave API do Gemini não configurada no ambiente (GEMINI_API_KEY)."
        )

    # Remove quebras de linha e espaços acidentais da chave
    key = key.strip()
    ...
    for modelo in MODELOS_ATIVOS:
        try:
            ...
            if resp.status_code == 200:
                ...
                erros.append(f"{modelo}: HTTP {resp.status_code} - {resp.text}")
...
app = FastAPI(title="NutriCore Pro Engine", version="19.0.0")
...
async def analisar_protocolo(request: Request, authorization: Optional[str] = Header(None)):
    ...
    res["status"] = "success"
    res["gerado_por"] = "Gemini 2.5 Flash"
