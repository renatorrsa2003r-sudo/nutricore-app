...
def init_db():
    ...
    # Dados Sincronizados do Usuário (Perfil, Dieta, Evolução)
    c.execute('''
...
# ==============================================================================
# 4. MOTOR INTELIGENTE DE IA (GEMINI MULTI-MODELOS)
# ==============================================================================
...
def executar_chamada_ia(prompt: str, chave_api: Optional[str] = None):
    ...
    # Tentativa 1: REST Oficial em cascata de modelos
...
app = FastAPI(title="NutriCore Pro Enterprise Engine", version="6.5.0")
...
def verificar_status_pagamento(payment_id: str):
    ...
    conn.close()
    return {"status": status, "is_approved": status == "approved"}

@app.all("/api/v1/payment/simulate-approval/{payment_id}")
...
def simular_aprovacao(payment_id: Optional[str] = None, authorization: Optional[str] = Header(None)):
    user = get_user_by_token(authorization)
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    if payment_id:
        ...
        row = c.fetchone()
        if row and row[0]:
            c.execute("UPDATE users SET subscription_status = 'active', plan_type = ?, is_pro = 1, subscription_end = ? WHERE id = ?", (row[1] or 'anual', sub_end, row[0]))
    else:
    ...
    if user:
        ...
        c.execute("UPDATE users SET subscription_status = 'active', is_pro = 1 WHERE is_pro = 0")
    ...
    return {
        ...
        "is_approved": True,
        "is_pro": True,
        "message": "Pagamento aprovado e plano PRO liberado com sucesso!"
    }
...
async def criar_plano(request: Request):
    ...
    # Aceita tanto o PerfilUsuarioInput detalhado quanto nomes em inglês/português simples
    ...
    # Cria objeto compatível para calcular metas
    ...
    # Fallback Determinístico Científico
    default_refeicoes = [
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app:app", host="0.0.0.0", port=int(os.getenv("PORT", 8```$", "", texto)
    return json.loads(texto.strip())
...
def capturar_lead_quiz(lead: LeadCaptureInput):
    ...
    if not clean_phone.startswith('55'):
        clean_phone = '55' + clean_phone
    msg = f"Olá {lead.name}! Seu diagnóstico no NutriCore Pro está pronto: https://nutricore-app-1.onrender.com"
...
def criar_pagamento_pix(dados: CreatePixPaymentInput, authorization: Optional[str] = Header(None)):
    ...
    if mp_access_token and len(mp_access_token) > 15:
        try:
            ...
            resp = requests.post("https://api.mercadopago.com/v1/payments", headers=headers, json=body, timeout=8)
            if resp.status_code in [200, 201]:
                data_mp = resp.json()
                payment_id = str(data_mp.get("id"))
                poi = data_mp.get("point_of_interaction", {}).get("transaction_data", {})
    ...
    qr_img_url = f"https://api.qrserver.com/v1/create-qr-code/?size=300x300&data={urllib.parse.quote(qr_code)}"
    agora = datetime.utcnow().isoformat()
...
def verificar_status_pagamento(payment_id: str):
    ...
    if mp_access_token and not payment_id.startswith("demo_") and status == "pending":
        try:
            headers = {"Authorization": f"Bearer {mp_access_token}"}
            resp = requests.get(f"https://api.mercadopago.com/v1/payments/{payment_id}", headers=headers, timeout=6)
            if resp.status_code == 200:
                status_mp = resp.json().get("status")
                if status_mp == "approved":
                    status = "approved"
