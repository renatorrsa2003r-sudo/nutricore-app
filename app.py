...
import requests
from fastapi import FastAPI, HTTPException, Request,import os
import re
import time
import json
import math
...
def get_db_conn():
    if not SUPABASE_URL:
        raise Exception("Variável SUPABASE_DATABASE_URL não configurada no ambiente.")
...
app = FastAPI(title="NutriCore Pro Engine", version="28.0.0")
...
def health():
    return {
        "status": "online",
        "database": "Supabase PostgreSQL" if SUPABASE_URL else "ERRO",
...
# --- AUTENTICAÇÃO E CRIAÇÃO DE CONTA (SUPABASE SEGURO) ---
...
# --- PAGAMENTOS PIX E SIMULAÇÕES PRO ---
    ...
    try:
        ...
        if MERCADO_PAGO_TOKEN and len(MERCADO_PAGO_TOKEN) > 15:
            try:
                ...
                body = {"transaction_amount": valor, "description": descricao, "payment_method_id": "pix", "payer": {"email": user_email, "first_name": user_name.split()[0] if user_name else "Cliente"}}
        ...
        return {
            ...
            "qr_code_url": qr_img_url, "ticket_url": qr_img_url, "copia_e_cola": qr_code, "pix_code": qr_code
        }
    except Exception as e:
...
async def simular_aprovacao_com_id(payment_id: str, authorization: Optional[str] = Header(None)):
    try:
        ...
        return {
            "status": "success", "approved": True, "is_approved": True, "is_pro": True,
            "subscription_status": "active", "plan_type": "anual", "message": f"Pagamento {payment_id} aprovado com sucesso! Acesso PRO liberado."
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def simular_aprovacao_sem_id(request: Request, authorization: Optional[str] = Header(None)):
    try:
        ...
        if payment_id: c.execute("UPDATE orders SET status = 'approved' WHERE payment_id = %s", (payment_id,))
        else: c.execute("UPDATE orders SET status = 'approved' WHERE status = 'pending'")
        ...
        return {
            "status": "success", "approved": True, "is_approved": True, "is_pro": True,
            "subscription_status": "active", "plan_type": "anual", "message": "Simulação de teste concluída com sucesso! Acesso PRO liberado."
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def criar_plano(request: Request):
    try:
        ...
        perfil = PerfilUsuarioInput(
            idade=idade, sexo=SexoEnum.FEMININO if sexo in ["feminino", "f", "female"] else SexoEnum.MASCULINO,
            peso_kg=peso_kg, altura_cm=altura_cm,
            ...
            orcamento=orcamento_enum, refeicoes_por_dia=max(3, min(6, refeicoes_por_dia)), dias_plano=max(1, min(30, dias_plano)),
        ...
        diretrizes_orcamento = {
            "economico": "DIRETRIZ FINANCEIRA RIGOROSA: ORÇAMENTO ECONÔMICO (R$ 12 a R$ 18/dia). Selecione APENAS alimentos de altíssimo custo-benefício: ovos, peito/coxa de frango, moela, fígado, sardinha fresca ou em lata. Carboidratos: arroz, feijão, aveia, banana, cuscuz, mandioca. PROIBIDO prescrever salmão ou queijos caros.",
            "moderado": "DIRETRIZ FINANCEIRA: ORÇAMENTO EQUILIBRADO (R$ 22 a R$ 35/dia). Proteínas: patinho moído, filé de tilápia, peito de frango, queijo minas, iogurte natural. Carboidratos e gorduras: arroz integral, batata doce, azeite, chia e frutas da estação.",
            "premium": "DIRETRIZ FINANCEIRA: ORÇAMENTO GOURMET / LIVRE (R$ 45+/dia). Priorize sofisticação e máxima densidade de micronutrientes: salmão fresco, filé mignon, camarão, queijo de cabra, iogurte grego artesanal, quinoa real, aspargos, mirtilos e castanhas nobres."
        }

        instrucao_orcamento = diretrizes_orcamento.get(perfil.orcamento.value, diretrizes_orcamento["economico"])

        prompt = f"""
        Você é um nutricionista clínico de elite. Elabore em TEMPO REAL um plano alimentar para {perfil.dias_plano} dia(s).
        DADOS: Sexo: {perfil.sexo.value}, Idade: {perfil.idade} anos, Peso: {perfil.peso_kg}kg, Altura: {perfil.altura_cm}cm
        Objetivo: {perfil.objetivo.value}. Refeições por Dia: {perfil.refeicoes_por_dia}.
        ...
        {instrucao_orcamento}
        Preferência: {preferencia} | Culinária: {estilo_culinario}
        Alimentos Favoritos: {favs if favs else 'Sem restrição'}
        Alimentos a Evitar: {evitar if evitar else 'Nenhum'}
        Restrições de Saúde: {', '.join(restricoes) if restricoes else 'Nenhuma'}
        ...
        if isinstance(resultado_json, list): lista_dias_raw = resultado_json
        elif isinstance(resultado_json, dict) and "dias" in resultado_json: lista_dias_raw = resultado_json["dias"]
        else: lista_dias_raw = [{"dia": 1, "titulo_dia": f"Dia 1 - Plano {perfil.orcamento.value.capitalize()}", "refeicoes": resultado_json.get("refeicoes", [])}]
        ...
        return {
            ...
            "tmb": tmb, "tdee": tdee, "meta_calorica": meta_calorica, "calorias_totais": meta_calorica,
            "budget_tier": perfil.orcamento.value, "macros": macros.dict(),
            "dias_total": len(dias_objs), "dias": [d.dict() for d in dias_objs],
            "refeicoes": [r.dict() for r in primeiro_dia_refeicoes], "meals": [r.dict() for r in primeiro_dia_refeicoes], "cardapio": [r.dict() for r in primeiro_dia_refeicoes]
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def admin_portal(request: Request, senha: Optional[str] = None):
    ...
    try:
        ...
        return f"Erro ao acessar painel admin: {str(e)}"

if __name__ == "__main__":
    import uvicorn
    uvicorn.run("app:app", host="0.0.0.0", port=int(os.getenv("PORT", 8000)), reload=True) Response, Header, Depends, status
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import FileResponse, JSONResponse, HTMLResponse
from pydantic import BaseModel, Field
from google import genai
