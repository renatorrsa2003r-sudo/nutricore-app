...
# --- 1. MODELOS DE DADOS DO PERFIL AVANÇADO ---
...
class PerfilUsuarioInput(BaseModel):
    ...
    ritmo_objetivo: Optional[str] = "moderado"  # conservador, moderado, agressivo
    ...
    horario_treino: Optional[str] = "nenhum"  # manha, tarde, noite, nenhum
    habilidade_culinaria: Optional[str] = "pratico"  # iniciante, pratico, gourmet
    orcamento: Optional[str] = "medio"  # economico, medio, premium
...
# --- 3. CÁLCULO BIOMÉTRICO CALIBRADO ---
def calcular_metas(p: PerfilUsuarioInput):
    ...
    # Ajuste fino por ritmo de agressividade
...
# --- 4. GERAÇÃO DE PLANO ALIMENTAR HIPER-PERSONALIZADO ---
def gerar_com_gemini(perfil: PerfilUsuarioInput, meta_calorica: float, macros: Macronutrientes, api_key: str) -> List[RefeicaoIA]:
    ...
    intolerancias_str = ", ".join(perfil.intolerancias_saude) if perfil.intolerancias_saude else "Nenhuma restrição clínica ou intolerância informada."

    prompt = f"""
    Você é um Nutricionista Clínico Esportivo e Chef de Gastronomia Funcional renomado.
    Crie um plano alimentar ultra-personalizado, respeitando a rotina biológica e clínica do paciente.
    ...
    CRONONUTRIÇÃO & ROTINA DIÁRIA:
    - Horário de Acordar: {perfil.horario_acordar} | Horário de Dormir: {perfil.horario_dormir}
    - Período do Treino: {perfil.horario_treino}
    
    RESTRIÇÕES CLÍNICAS & INTOLERÂNCIAS (OBRIGATÓRIO RESPEITAR):
    - {intolerancias_str}
    - Alimentos Favoritos: {perfil.alimentos_favoritos if perfil.alimentos_favoritos else 'Variados'}
    - Alimentos para EVITAR: {perfil.alimentos_evitar if perfil.alimentos_evitar else 'Nenhum'}

    METAS DO DIA ({perfil.refeicoes_por_dia} refeições):
    - Calorias Totais: {meta_calorica} kcal
    - Proteínas: {macros.proteinas_g}g | Carboidratos: {macros.carboidratos_g}g | Gorduras: {macros.gorduras_g}g

    DIRETRIZES FUNDAMENTAIS:
    1. Organize os horários de forma coerente entre {perfil.horario_acordar} e {perfil.horario_dormir}.
    2. Se o treino for informado ({perfil.horario_treino}), marque a refeição correspondente com "(Pré-Treino)" ou "(Pós-Treino)" e adapte o perfil nutricional.
    3. RESPEITE RIGOROSAMENTE todas as intolerâncias clínicas informadas (ex: se houver Sem Glúten ou Gastrite, adeque ingredientes e temperos).
    4. Respeite as gramaturas exatas para o peso corporal de {perfil.peso_kg}kg.

    Retorne APENAS um array JSON contendo exatamente {perfil.refeicoes_por_dia} objetos.
    Formato de cada objeto:
    {{
      "nome_refeicao": "Ex: Café da Manhã (Pré-Treino)",
      "titulo_prato": "Nome gastronômico apetitoso",
      ...
      "ingredientes": ["Ex: 150g de Filé de frango marinado com ervas", "Ex: 130g de Batata doce grelhada"],
      "modo_preparo": "Instruções claras e sucintas de preparo.",
      "dica_chef": "Dica nutricional funcional ou gastronômica."
    }}
    """
...
def gerar_consulta_funcional(dados_input: ConsultaFuncionalInput, api_key: str) -> ConsultaFuncionalResponse:
    ...
    O paciente solicitou uma solução nutricional para o seguinte objetivo / queixa específica:
    ...
    2. Explicação fisiológica sucinta sobre mecanismos de ação e compostos bioativos.
    3. Lista de 4 a 6 Alimentos-Chave (com porção sugerida, por que funciona bioquimicamente e como consumir).
    4. Lista de 3 a 5 Alimentos/Hábitos para EVITAR.
    5. Uma Receita Terapêutica Rápida (shot, smoothie ou chá funcional de 2 a 5 minutos).
    ...
    {{
      ...
      "alimentos_chave": [
        {{
          ...
          "porcao_sugerida": "Porção exata",
          "por_que_funciona": "Mecanismo biológico",
